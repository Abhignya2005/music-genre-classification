...
def welcome():
    return render_template_string('''
        ...
        window.onload = function() {
            ...
            // Show the "WELCOME TO THE AUTOMATIC MUSIC CLASSIFICATION" message
            setTimeout(() => {
                book.classList.add('open');
                let msg = new SpeechSynthesisUtterance("Welcome to Automatic Music Classification");
                msg.rate = 1;
            ...
            // After 10 seconds, show the overlay with key press hint
            setTimeout(() => {
                overlay.classList.add('visible');
            }, 1000);

            // After 11 seconds, show "Evaluation" text
            setTimeout(() => {
                evaluationText.style.opacity = 1;
                let msg = new SpeechSynthesisUtterance("Evaluation");
                msg.rate = 1;
                msg.pitch = 1;
                window.speechSynthesis.speak(msg);
            }, 1100);

            // After 18 seconds (when Evaluation disappears), show file upload form centered
            setTimeout(() => {
                evaluationText.style.opacity = 0; // Make Evaluation text disappear
                fileUploadForm.style.display = 'block'; // Show the file upload form
            }, 1100);  // Delay for the duration of the Evaluation text display
        };
    </script>
        </body>
        </html>
...
def predict():
    ...
    <body>
        <h1>Predicted Genre: {{ genre }}</h1>
        <p>Press any key to go to Thank You page.</p>
        <script>
            document.addEventListener('keydown', function() {
...
def thank_you():
    ...
    <body>
        <h1>Thank You for Using the Music Classification System!</h1>
    </body>
    </html>
    ''')
