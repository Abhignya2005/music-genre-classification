...
model_save_path = 'music_genre_model.pth'  # File name for the saved model
torch.save(model.state_dict(), model_save_path)
print(f"Model saved successfully at {model_save_path}")

# Step 2: Save class labels
#import json
# Create a mapping from integer labels to genre names
label_mapping = {i: genre for i, genre in enumerate(y.unique())}

# Save the mapping as a JSON file
with open('class_labels.json', 'w') as f:
    json.dump(label_mapping, f)

print("Class labels saved successfully as class_labels.json")